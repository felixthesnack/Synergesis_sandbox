...
public class CardDatabase : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        //jsonCards = GetComponent<JSON_test>();

        //for(int i = 0; i < 6; i++)
        //{
        //    tier1Starter.Add(jsonCards.cardList.JSONCards[i]);
        //    Debug.Log("Added " + jsonCards.cardList.JSONCards[i].cardName + " card");
        //}
    ...
    IEnumerator GetJSONCardList()
    {
        ...
        else
        {
            ...
            for (int i = 6; i < 15; i++)
            {
                CardDatabase.tier1Basic.Add(cardList.JSONCards[i]);
                Debug.Log("Added " + cardList.JSONCards[i].id + " card");
            }

            for (int i = 15; i < 31; i++)
            {
                CardDatabase.tier1Special.Add(cardList.JSONCards[i]);
                Debug.Log("Added " + cardList.JSONCards[i].id + " card");
            }

            for (int i = 31; i < 40; i++)
            {
                CardDatabase.tier1Skills.Add(cardList.JSONCards[i]);
                Debug.Log("Added " + cardList.JSONCards[i].id + " card");
            }
